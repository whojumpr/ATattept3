// This file serves as the main API entrypoint for Vercel
...
app.get('/api/healthcheck', (req, res) => {
  res.status(200).json({ status: 'ok', message: 'API is functioning correctly' });
});
...
export default function handler(req, res) {
  // Properly parse path parameters for API routes
  // For example, /api/trades/123 will have req.url = /trades/123
  // We need to adjust for this in our Express handler
  if (req.url.startsWith('/')) {
    // This is needed because Vercel strips the /api prefix
    // from the URL when it's sent to the API handler
    req.url = '/api' + req.url;
  }
